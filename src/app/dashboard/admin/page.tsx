...
export default function AdminPage() {
  ...
  const [roleChangePending, setRoleChangePending] = useState<{ uid: string; name: string; newRole: "admin" | "user" | "view" } | null>(null);
  ...
  const handleRoleChangeRequest = (uid: string, name: string, newRole: "admin" | "user" | "view") => {
    if (uid === user?.uid) {
                  ...
                  <tr key={u.uid} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '1rem' }}>
                      <span style={{
                        ...
                        fontWeight: 700,
                        textTransform: 'uppercase',
                        backgroundColor: u.role === 'admin' ? '#FEE2E2' : u.role === 'view' ? '#F3F4F6' : '#E0F2FE',
                        color: u.role === 'admin' ? '#EF4444' : u.role === 'view' ? '#6B7280' : '#0369A1'
                      }}>
                        {u.role}
                      </span>
                    </td>
